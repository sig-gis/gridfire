...
{:paths ["src" "resources"]
 ...
 :deps {org.clojure/clojure                 {:mvn/version "1.10.1"}
        ...
        sig-gis/magellan                    {:mvn/version "20200909.105353"}
...
To initialize an existing burn paremeter from a raster we have two options. We can
read rasters from a PostGIS database or a raster file on disk.

To initialize a burn paremeter from a raster in our PostGIS database include the
following mappings:
- *fetch-ignition-method*: a keyword to denote which method we want to use
- *ignition-layers*: a mapping of layer names to a partial sql statement

#+begin_src clojure
{:fetch-ignition-method :postgis
 :ignition-layers       {:initial-fire-spread         "ignition.scar WHERE rid=1"
                         :initial-fire-line-intensity "ignition.ifi WHERE rid=1"
                         :initial-flame-length        "ignition.ifl WHERE rid=1"}}
#+end_src

To initialize a burn paremeter from a raster file on disk include the
following mappings:
- *fetch-ignition-method*: a keyword to denote which method we want to use
- *ignition-layers*: a mapping of layer names to filepaths

#+begin_src clojure
{:fetch-ignition-method :geotiff
 :ignition-layers       {:initial-fire-spread         "test/gridfire/resources/scar.tif"
                         :initial-fire-line-intensity "test/gridfire/resources/ifi.tif"
                         :initial-flame-length        "test/gridfire/resources/ifl.tif"}}
#+end_src

Gridfire makes use of clojure's multimethods to dispatch control to
different handlers for fetching ignition layers. The dispatch depnds 
on what is in the config file. Here's the namescapce that implements
this functionality.
...
initializing a burn paremeter from reading a raster file from disk.

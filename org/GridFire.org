...
- *<weather-type>*: a map of location and additional options

#+begin_src clojure
{:fetch-temperature-method :postgis
 :temperature              {:sql "weather.tmpf WHERE rid=1"}}
#+end_src
...
for inherent uncertainty in the data. A random sampling procedure is implemented such that
the spatial perturbation to be applied to a given input raster sampled from a
probability density function (pdf). Currenlty only a uniform distribution is supported.

...
To initialize an existing burn parameter from a raster we have two options. We can
read rasters from a PostGIS database or a raster file on disk.

To initialize a burn parameter from a raster in our PostGIS database include the
following mappings:
...
To initialize a burn parameter from a raster file on disk include the
